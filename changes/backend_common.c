...
int read_psrdata_mask(float *fdata, int numspect, struct spectra_info *s,
                      int *delays, int *padding,
                      int *maskchans, int *nummasked, mask * obsmask)
// (RSW 23/11/17) Modified version of read_psrdata() that writes out 
// the raw masked data.  Goal is to recreate the behavior of 
// read_PSRFITS_mask() used for Laura Spitler's modulation index code.
// 
// This routine reads numspect from the raw pulsar data defined in
// "s". Time delays and a mask are applied to each channel.  It
// returns the # of points read if successful, 0 otherwise.  If
    ...
    *nummasked = 0;
    ...
    if (allocd) {
        while (1) {
            ...
            // NEW
            //chkfwrite(currentdata, sizeof(char), numread * S.bytes_per_subint/S.num_polns, fout);
            chkfwrite(currentdata, sizeof(float), numread * s->spectra_per_subint * s->num_channels, fout);
